...
public class Pets {
    public static void main(String[] args) {
        ...
        for(Pet p: pets){
            if(p instanceof Cat){
                System.out.printf("Name of pet: %s \tType: Cat \tCoat Color: %s\n", p.getName(), ((Cat) p).getCoatColor());
            } else if (p instanceof Dog) {
                System.out.printf("Name of pet: %s \tType: Dog \tWeight: %.1f\n", p.getName(), ((Dog) p).getWeight());
            }
        }
        scan.close();
    }
